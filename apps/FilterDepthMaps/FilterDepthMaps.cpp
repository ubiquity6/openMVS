// Based on DensifyPointCloud.cpp
...
bool Initialize(size_t argc, LPCTSTR* argv, int &nStartFrame, int &nEndFrame)
{
    ...
    ("start-frame,d", boost::program_options::value<int>(&nStartFrame)->default_value(-1), "The frame at which to start. If not specified, all frames are done.")
    ("end-frame,e", boost::program_options::value<int>(&nEndFrame)->default_value(-1), "The frame at which to end. If not specified, all frames are done.")
		;
  if (nEndFrame == -1 && nStartFrame != -1)
...
int main(int argc, LPCTSTR* argv)
{
  ...
  int nStartFrame, nEndFrame;
	if (!Initialize(argc, argv,nStartFrame,nEndFrame))
		return EXIT_FAILURE;
	...
	if ((ARCHIVE_TYPE)OPT::nArchiveType != ARCHIVE_MVS) {
		TD_TIMER_START();
		if (!scene.FilterDepthMaps(nStartFrame,nEndFrame))
			return EXIT_FAILURE;
		VERBOSE("Filtration of depth maps complete.");
	}
	Finalize();
	return EXIT_SUCCESS;
}
